...
// Complete the exercise in the space below:
sortedByBirthYear = inventors.sort(function(a, b) {
...
// Complete the exercise in the space below:
inventorNamedAda = inventors.find(function(findName) {
    return findName.first === 'Ada';
});
...
firstLast = people.map(function(byName) {
    
    let [last, first] = byName.split(', ');
    return `${first} ${last}`;
});
...
commentById = comments.find(function(findID) {
    return findID.id === 823423;
});
...
idx = comments.findIndex(function(findIndex) {
    return findIndex.id === 123523;
});
...
travelMethodCounts = travelMethods.reduce((accumulator, methodCount) => {
    accumulator[methodCount] = (accumulator[methodCount] || 0) + 1;
    return accumulator;
}, {});
